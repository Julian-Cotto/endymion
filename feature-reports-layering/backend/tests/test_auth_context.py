...
def test_mock_context_defaults():
    ...
    assert "reports-layering.view" in ctx.roles


def test_debug_roles_override():
    ctx = extract_auth_context(
        authorization_header=None,
        debug_roles="reports-layering.view,reports-layering.edit",
    )

    assert ctx.is_authenticated is True
    assert "reports-layering.view" in ctx.roles
    assert "reports-layering.edit" in ctx.roles


def test_roles_are_permission_shaped():
    ctx = extract_auth_context(
        authorization_header=None,
        debug_roles="reports-layering.view",
    )

    # No mapping should happen
    assert ctx.roles == ["reports-layering.view"]
