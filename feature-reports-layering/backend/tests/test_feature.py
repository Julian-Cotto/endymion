...
def test_health():
    ...
    assert response.json()["feature_key"] == "reports-layering"


def test_me_allowed_with_default_mock_role(monkeypatch):
    monkeypatch.setenv("AUTH_MODE", "mock")
    monkeypatch.setenv("AUTH_DEBUG_HEADERS_ENABLED", "true")
    monkeypatch.setenv("AUTH_DEFAULT_DEV_ROLES_RAW", "reports-layering.view")
    monkeypatch.setenv("AUTH_REQUIRED_PERMISSIONS_RAW", "reports-layering.view")
    ...
    assert "reports-layering.view" in body["roles"]
...
def test_items_allowed_with_debug_role(monkeypatch):
    ...
    response = client.get(
        "/api/reports/items",
        headers={
            "X-Debug-Roles": "reports-layering.view",
        },
    )

    assert response.status_code == 200
    assert response.json()["featureKey"] == "reports-layering"
...
def test_items_denied_without_required_permission(monkeypatch):
    ...
    response = client.get("/api/reports/items")
