...
def normalize_auth(manifest: dict) -> None:
    auth = manifest.get("auth")

    if not isinstance(auth, dict):
        auth = {}

    mode = str(auth.get("mode") or os.getenv("FEATURE_AUTH_MODE", "mock")).strip().lower()
    ...
    if mode == "entra":
        auth["required"] = True
        auth["mode"] = "entra"
        auth["shellAuthRequired"] = True
        auth["tokenForwarding"] = True
        auth["tokenStrategy"] = "forwarded-bearer"
        auth["allowedDevModes"] = auth.get("allowedDevModes") or ["mock"]
        auth["roles"] = []
    elif mode == "none":
        auth["required"] = False
        auth["mode"] = "none"
        auth["shellAuthRequired"] = False
        auth["tokenForwarding"] = False
        auth["tokenStrategy"] = "none"
        auth["allowedDevModes"] = auth.get("allowedDevModes") or ["mock"]
        auth["roles"] = []
    else:
        auth["required"] = True
        auth["mode"] = "mock"
        auth["shellAuthRequired"] = bool(auth.get("shellAuthRequired", True))
        auth["tokenForwarding"] = bool(auth.get("tokenForwarding", False))
        auth["tokenStrategy"] = auth.get("tokenStrategy") or "none"
        auth["allowedDevModes"] = auth.get("allowedDevModes") or ["mock"]
        auth["roles"] = []
